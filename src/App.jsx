Warning: truncated output (original token count: 68698)
Total output lines: 5054
...
// Les profils et leurs plans sont chargés après authentification depuis Supabase.
// Rien de nominatif ou de médical n'est intégré au JavaScript public.
const PROFILES = [];
const MODES_BY_PROFILE = {};
const BASE_PROFILE = {};
const PLAN_BY_USER_ID = {};
const DEFAULT_MODE_BY_PROFILE = {};
...
function registerNutritionProfile(nutritionProfile) {
  ...
  BASE_PROFILE[profileId] = {
    ...
    profile: 'Programme nutritionnel personnel. Respecter les contraintes indiquées dans le mode actif.',
  };
  DEFAULT_MODE_BY_PROFILE[profileId] = nutritionProfile.calibration_json?.defaultMode
    || modes.find(mode => mode.id === 'standard')?.id
    || modes[0]?.id;
  for (const mode of modes) {
    ...
    USERS[uid] = {
      ...
      profile: `${BASE_PROFILE[profileId].profile} Mode actif : ${mode.label} (${mode.desc}). ${mode.coachProfile || ''}`.trim(),
      plan: mode.plan || [],
    };
  }
}
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h2 className="font-bold text-slate-800 text-sm flex items-center gap-1.5 flex-wrap">
              {meal.name}
              {meal.suggestedTime && <span className="rounded-full bg-white/80 px-1.5 py-0.5 text-[9px] font-bold text-violet-700">vers {meal.suggestedTime}</span>}
...
10. Si un contexte santé est fourni, prends-le en compte pour éviter les conseils incompatibles. Ne pose aucun diagnostic, ne modifie jamais un traitement et conseille un avis médical lorsqu’une situation ou un symptôme persistant n’a jamais été évalué.`;
  ...
  const applyModeSwitch = (modeId) => {
    ...
    if (hasConsumed) {
      const transferred = transferModeConsumption({
        sourcePlan: plan,
        sourceStatus: status,
        sourceRealQty: realQty,
        destinationPlan: USERS[nextUserId].plan,
        consumed,
        currentMode,
      });
      transferred.realQty.__modeCarryover.fromModeLabel = user.modeLabel;
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-3 sm:p-6">
      ...
      <div style={{display: activeSection !== 'journal' ? 'none' : 'block'}}>
        ...
        {currentMode === 'deficit' && nutritionProfile?.calibration_json?.deficitProtocol?.status === 'active' && (
          <div className="mb-3 rounded-2xl border border-amber-300 bg-amber-50 px-4 py-3 text-xs leading-relaxed text-amber-950" role="status">
            <strong>📉 Protocole spécifique de 7 jours uniquement.</strong> Il ne doit pas être prolongé. Fin automatique le {new Date(nutritionProfile.calibration_json.deficitProtocol.endsAt).toLocaleDateString('fr-CH')} ; le mode Standard redeviendra ensuite la référence.
          </div>
        )}
              ...
              <div className="space-y-2">
                {plan.map(meal => (
                  <MealCard
                    ...
                    accent={ACCENT_THEME_BY_PROFILE[currentProfile] || DEFAULT_ACCENT_THEME}
                  />
                ))}          {(() => {
          ...
          <div className="mt-4 pt-4 border-t border-slate-100">
            ...
            <div className="flex gap-2">
              ...
              <input
                ...
                placeholder={'Ex : "J’ai sauté mon goûter"'}
