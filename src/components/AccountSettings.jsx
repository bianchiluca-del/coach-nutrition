...
export default function AccountSettings({ session, profileName, syncState, onSignOut, signingOut, isCoach = false, onOpenCoach }) {
  ...
  const changePassword = async (event) => {
    ...
    const policyError = passwordValidationMessage(newPassword);
    if (policyError) {
      setPasswordNotice({ type: 'error', text: policyError });
      return;
    }
    ...
    <div className="space-y-4 pb-28 pt-2">
      ...
      {isCoach && (
        <section className="rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-50 to-white p-4 shadow-sm">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1"><h3 className="font-black text-slate-900">Pilotage de la bêta</h3><p className="mt-1 text-xs leading-relaxed text-slate-600">Crée les 10 invitations, suis leur activation et ouvre uniquement les dossiers des clients qui t’ont donné leur accord.</p></div>
          </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <form className="space-y-3" onSubmit={changePassword}>
          ...
          <input type={passwordType} autoComplete="new-password" minLength={12} className={inputClass} value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="12 caractères, majuscule, chiffre et symbole" />
